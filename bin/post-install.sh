...
# --------------------------------------------------------------------------------
# This script is doing the post-install processing for Alignak Web UI application:
# - checking and creating alignak:alignak user account
# - installing the python dependencies for Alignak Web UI.
# -----
...
then
   ...
   then
      ...
      echo "You can then run: sudo $PREFIX/share/alignak-webui/post-install.sh"
      exit 1
   fi
else
   ...
   if [ "$pyver" = "2.7" ]; then
      ...
      then
         ...
         echo "You can then run: $PREFIX/share/alignak-webui/post-install.sh"
         exit 1
      fi
   else
...
mkdir -p $PREFIX/var/log/alignak-webui
chown -R $ACCOUNT:$ACCOUNT $PREFIX/var/log/alignak-webui
echo "$ACCOUNT user and members of its group $ACCOUNT are granted 775 on $PREFIX/var/log/alignak-webui"
chmod -R 775 $PREFIX/var/log/alignak-webui
